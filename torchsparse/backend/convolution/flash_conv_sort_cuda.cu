...
__device__ void load_reg_A_k32n64(uint32_t* reg_A, half* shm_A, int ki) {
    for (int m = 0; m < 4; m++) {
        int lane_id = threadIdx.x;
        int row = threadIdx.z * 64 + m * 16 + lane_id % 16;
        int col = ki * 16 + lane_id / 16 * 8;
...
__global__ void flash_conv_sort_k32n64(half* inputs, half* weights, int* reorder_map, int* reduced_mask, 
                                       int* reorder_loc, half* outputs, 
                                       int n_points, int c_in, int c_out, int kernel_size) {
    int M = n_points;
    int N = c_out;
    ...
    pipe_load_k32n64(shm_A, shm_B, inputs, weights, reorder_map, kernel_size, c_in, N, 0, 0);
    __pipeline_commit();
    int idx0 = 0;
    int loc0 = 0;
    int loc1;
    ...
    for (int ko = 1; ko < K / 32; ko++) {
        ...
        if (flag) {
            loc1 = loc0 ^ 1;
            pipe_load_k32n64(shm_A, shm_B, inputs, weights, reorder_map, kernel_size, c_in, N, ko, loc1);
            __pipeline_commit();
            __pipeline_wait_prior(1);
            pipe_calc_k32n64(shm_A, shm_B, reg_A, reg_B, reg_C, idx0, loc0);
            __syncthreads();
            idx0 = ko;
            loc0 = loc1;
        }
...
__device__ void load_shm_A_m64(half* shm_A, half* inputs, int* reorder_map, int kernel_size, int c_in, int ko) {
    // layout: [64, 64]
    int tid = threadIdx.z * 64 + threadIdx.y * 32 + threadIdx.x;
    for (int i = 0; i < 4; i++) {
        int row = i * 16 + tid / 8;
        int col = tid % 8 * 8;
        int row_A = reorder_map[(blockIdx.x * 64 + row) * kernel_size + (ko * 64) / c_in];
        int col_A = (ko * 64 + col) % c_in;
        int shm_row = row;
        int shm_col = col ^ ((shm_row & 7) << 3);
        if (row_A == -1) {
...
__device__ void load_shm_B_m64(half* shm_B, half* B, int K, int N, int ko) {
    // layout: [64, 64]
    int tid = threadIdx.z * 64 + threadIdx.y * 32 + threadIdx.x;
    for (int i = 0; i < 4; i++) {
        int row = i * 16 + tid / 8;
...
__device__ void load_reg_A_m64(uint32_t* reg_A, half* shm_A, int ki) {
    for (int m = 0; m < 2; m++) {
        int lane_id = threadIdx.x;
        int row = threadIdx.z * 32 + m * 16 + lane_id % 16;
        int col = ki * 16 + lane_id / 16 * 8;
        int shm_row = row;
        int shm_col = col ^ ((shm_row & 7) << 3);
        uint32_t shm_A_lane_addr = __cvta_generic_to_shared(shm_A + shm_row * 64 + shm_col);
        LDMATRIX_X4(reg_A[ki * 8 + m * 4], reg_A[ki * 8 + m * 4 + 1], reg_A[ki * 8 + m * 4 + 2], reg_A[ki * 8 + m * 4 + 3], shm_A_lane_addr);
    }
}

__device__ void load_reg_B_m64(uint32_t* reg_B, half* shm_B, int ki) {
    int lane_id = threadIdx.x;
    for (int ni = 0; ni < 2; ni++) {
        int row = ki * 16 + lane_id % 16;
        int col = threadIdx.y * 32 + ni * 16 + lane_id / 16 * 8;
...
__device__ void store_C_m64(uint32_t* reg_C, half* C, int* reorder_loc, int M, int N) {
    int lane_id = threadIdx.x;
    for (int m = 0; m < 2; m++) {
        for (int n = 0; n < 4; n++) {
            int shm_row = threadIdx.z * 32 + m * 16 + lane_id / 4;
            int shm_col = threadIdx.y * 32 + n * 8 + (lane_id % 4) * 2;
...
__device__ void pipe_load_m64(half* shm_A, half* shm_B, half* inputs, half* weights, int* reorder_map, 
                          int kernel_size, int c_in, int N, int ko, int loc) {
    shm_A += loc * 64 * 64;
    shm_B += loc * 64 * 64;
    load_shm_A_m64(shm_A, inputs, reorder_map, kernel_size, c_in, ko);
    load_shm_B_m64(shm_B, weights, kernel_size * c_in, N, ko);
}

__device__ void pipe_calc_m64(half* shm_A, half* shm_B, uint32_t* reg_A, uint32_t* reg_B, uint32_t* reg_C, int ko, int loc) {
    shm_A += loc * 64 * 64;
    shm_B += loc * 64 * 64;
    for (int ki = 0; ki < 4; ki++) {
        load_reg_A_m64(reg_A, shm_A, ki);
        load_reg_B_m64(reg_B, shm_B, ki);
    }
    ...
    for (int m = 0; m < 2; m++) {
        for (int ki = 0; ki < 4; ki++) {
            for (int n = 0; n < 4; n++) {
                int idx = m * 4 + n;
                HMMA16816(reg_C[idx * 4], reg_C[idx * 4 + 1], reg_C[idx * 4 + 2], reg_C[idx * 4 + 3],
                    reg_A[ki * 8 + m * 4], reg_A[ki * 8 + m * 4 + 1], reg_A[ki * 8 + m * 4 + 2], reg_A[ki * 8 + m * 4 + 3],
                    reg_B[ki * 8 + n * 2], reg_B[ki * 8 + n * 2 + 1],
                    reg_C[idx * 4], reg_C[idx * 4 + 1], reg_C[idx * 4 + 2], reg_C[idx * 4 + 3]);
            }
        }
...
__global__ void flash_conv_sort_m64(half* inputs, half* weights, int* reorder_map, int* reduced_mask, 
                                       int* reorder_loc, half* outputs, 
                                       int n_points, int c_in, int c_out, int kernel_size) {
    int M = n_points;
    int N = c_out;
    ...
    uint32_t reg_A[4 * 2 * 4];
    uint32_t reg_B[4 * 4 * 2];
    uint32_t reg_C[2 * 4 * 4] = {0};

    pipe_load_m64(shm_A, shm_B, inputs, weights, reorder_map, kernel_size, c_in, N, 0, 0);
    __pipeline_commit();
    int idx0 = 0;
    int loc0 = 0;
    int loc1;
    ...
    for (int ko = 1; ko < K / 64; ko++) {
        ...
        if (flag) {
            loc1 = loc0 ^ 1;
            pipe_load_m64(shm_A, shm_B, inputs, weights, reorder_map, kernel_size, c_in, N, ko, loc1);
            __pipeline_commit();
            __pipeline_wait_prior(1);
            pipe_calc_m64(shm_A, shm_B, reg_A, reg_B, reg_C, idx0, loc0);
            __syncthreads();
            idx0 = ko;
            loc0 = loc1;
        }
