...
__device__ void load_shm_A_sort(half* shm_A, half* inputs, int* reorder_map, int kernel_size, int c_in, int ko) {
...
__device__ void load_shm_B_sort(half* shm_B, half* B, int K, int N, int ko) {
    // layout: [32, 72]
    int tid = threadIdx.z * 64 + threadIdx.y * 32 + threadIdx.x;
    for (int i = 0; i < 2; i++) {
        int row = i * 16 + tid / 8;
...
__device__ void load_reg_A_sort(uint32_t* reg_A, half* shm_A, int ki) {
    for (int m = 0; m < 4; m++) {
...
__device__ void store_C_sort(uint32_t* reg_C, half* C, int* reorder_loc, int M, int N) {
    int lane_id = threadIdx.x;
    for (int m = 0; m < 4; m++) {
        for (int n = 0; n < 4; n++) {
