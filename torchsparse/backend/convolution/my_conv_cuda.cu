...
__device__ void load_shm_A(half *shm_A, half *A, int* out_in_map, int c_in, int kernel_volume, int ko)
{
    // layout: [64, 64]
    ...
    for (int i = 0; i < 8; i++) {
        ...
        int row_A = out_in_map[(blockIdx.x * 64 + row) * kernel_volume + (ko * 64 + col) / c_in];
...
__global__ void my_conv_kernel(int M, int c_in, int N, int kernel_volume, half *__restrict__ A, 
    ...
    for (int k = 0; k < K / 64; k++) {
        load_shm_A(shm_A, A, out_in_map, c_in, kernel_volume, k);
                        ...
                        int n_points, int c_out) 
{
    ...
    my_conv_kernel<<<dim3(cdiv(M, 64), cdiv(N, 64)), dim3(32, 2)>>>
                    (M, c_in, c_out, kernel_volume, in_feats, kernel, out_in_map, out_feats);
